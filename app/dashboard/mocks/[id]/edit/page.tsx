      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            setErrors({ submit: "Mock not found" })
          } else {
            setErrors({ submit: "Failed to load mock" })
          }
          setIsFetching(false)
          return
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/mocks/${mockId}`, {
        ...
        body: JSON.stringify({
          ...formData,
          responseBody: formData.responseBody, // Send as string, API will parse
        }),
      })

      if (!response.ok) {
        const errorData = await response.json()
        setErrors({ submit: errorData.error || "Failed to update mock API" })
        setIsLoading(false)
        return
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Edit Mock API</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 relative">
        ...
        <Card className="relative z-0 overflow-visible">
          <CardHeader>
            ...
            <CardDescription>Update your mock endpoint</CardDescription>
          </CardHeader>
          <CardContent>
